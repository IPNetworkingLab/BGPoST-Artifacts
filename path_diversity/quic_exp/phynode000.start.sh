...
ip netns exec node001 bash -c "taskset -c 1 /tmp/bird -s /tmp/node001.bird.sk -c "${ROOT}"/node001.bird.step.1.cfg -P /tmp/node001.bird.pid &"
...
ip netns exec node005 bash -c "sh /tmp/gobgp.sh node005"

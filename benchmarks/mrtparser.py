...
class Stats:
    ...
    def update_max_time(self, ts):
        if ts > self.max_time:
            self.max_time = ts

    def update_min_time(self, ts):
        if self.min_time < 0:
            self.min_time = ts
        elif ts < self.min_time:
            self.min_time = ts

    def update_min_max_time(self, ts):
        self.update_min_time(ts)
        self.update_max_time(ts)
...
def parse_BGP4MP(data, stats):
    if (BGP4MP_ST['BGP4MP_MESSAGE'] not in data['subtype'] and
            ...
            BGP4MP_ST['BGP4MP_MESSAGE_AS4_ADDPATH'] not in data['subtype'] and
            BGP4MP_ST['BGP4MP_MESSAGE_ADDPATH'] not in data['subtype']):
        return False
    ...
    for prefix in pfxs:
        ...
        assert 0 <= data['microsecond_timestamp'] < 1_000_000

        stats.update_min_max_time(timestamp)
        try:
...
def get_cdf_data(times: typing.Iterable):
    count1, b_count1 = np.histogram(times, bins=200000)
    pdf1 = count1 / sum(count1)
    cdf1 = np.cumsum(pdf1)
    return b_count1[1:], cdf1


def plot_cdf(times: typing.Iterable, legend: str, lstyle: str = '-'):
    x, y = get_cdf_data(times)
    plt.plot(x, y, label=legend, linestyle=lstyle)


def plot_show(x_legend: str, save: Union[str | None], boxplot: bool):
    if boxplot:
        plt.ylabel(x_legend)
    else:
        plt.ylabel('CDF')
...
def parse_mrt_dump(mrt_path: str, stats: Stats):
...
def plot_mrt_dump(mrt_path: str, legend: str, out_store: Union[None, Path] = None, lstyle: str = '-'):
    stats = Stats()

    parse_mrt_dump(mrt_path, stats)
    ...
    plot_cdf([times[-1][1] - times[0][1] for pfx, times in stats.pfx_seen.items() if len(times) == 2],
             legend=legend, lstyle=lstyle)


def split_input(entry: str):
    splitted = entry.split(',')
    if len(splitted) != 2 and len(splitted) != 3:
        raise ValueError("Bad input format. Usage: -i <mrt_file_path>,<legend>[,tick] "
                         "or -c <csv_file_path>,<legend>[,tick].")

    if len(splitted) == 2:
        splitted.append('solid')
...
def do_boxplot(datas, legends):
    plt.boxplot(datas, labels=legends, showfliers=False)
...
def main(args):
    ...
    if args.inputs:
        for record in args.inputs:
            mrt_file, legend, linestyle = split_input(record)
            print(f"Parsing {mrt_file} ({legend})")
            dest_file = out_dir.joinpath(f"{get_file_stem(mrt_file)}.csv.gz") if out_dir else None
            if args.boxplot:
                stats = Stats()
                parse_mrt_dump(mrt_file, stats)
                bp_data.append([times[-1][1] - times[0][1] for pfx, times in stats.pfx_seen.items() if len(times) == 2])
                bp_legend.append(legend)
            else:
                plot_mrt_dump(mrt_file, legend, dest_file, lstyle=linestyle)

    if args.boxplot:
        do_boxplot(bp_data, bp_legend)

    plot_show("Prefix Propagation Duration (ms)", args.save, args.boxplot)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--boxplot', action='store_true',
                        help="Do boxplot instead of CDF")
